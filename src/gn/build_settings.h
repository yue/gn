...
class BuildSettings {
 public:
  ...
  bool IsChromiumPath(const Arg& path) const {
    return IsChromiumPath(path.value());
  }

  // Returns the relative path inside source root, or absolute path if outside
  // the source root.
  std::string TranslateChromiumFullPath(const base::FilePath& path) const;
  template<typename Arg>
  std::string TranslateChromiumPath(Arg&& path) const {
    return TranslateChromiumFullPath(GetFullPathChromium(
        std::forward<Arg>(path)));
  }
 ...
 private:
  ...
  bool use_chromium_config_ = false;
